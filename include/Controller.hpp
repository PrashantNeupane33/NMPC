...
private:
  ...
  std::tuple<double, double, double, double>
      weights;           //(Q0,Qother,predWeight,rateWeight)
  ...
  std::tuple<MatrixXd, MatrixXd> linearizeModel(const VectorXd &x_bar,
                                                const VectorXd &u_bar) const;
  ...
  getWeightMatrices(std::tuple<double, double, double> weights);
  void writeToCSV(const std::string &filename, const MatrixXd &matrix) const;

public:
  MPC(MatrixXd _C, std::tuple<unsigned int, unsigned int> horizons,
      std::tuple<double, double, double, double> weights, double _sampling,
      VectorXd _u_min, VectorXd _u_max);
